...
2. `www/` → 루트 미러 (index/style + 분리된 JS 6개 모두):
   ```
   cp www/index.html index.html && cp www/style.css style.css && cp www/app.core.js app.core.js && cp www/app.map.js app.map.js && cp www/app.rescue.js app.rescue.js && cp www/app.report.js app.report.js && cp www/app.ops.js app.ops.js && cp www/app.js app.js && cp www/park-boundary.json park-boundary.json
   ```
3. feature 브랜치에 push
4. **즉시 main에 머지·푸시**:
   ```
