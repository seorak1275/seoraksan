...
- **app.map.js** (1.9k줄): `openApp`/`switchTab`(하단탭 nv1~4, nv4=담당 업무함 표시조건) · `showV`/`rMaps` · 상황판(`openBoard` — v-board를 body로 이동, `_createBoardMap`, `toggleBoardMapType`) · 공원경계(`_drawParkBoundary`) · 오프라인 지도 미리받기(`preloadParkTiles` — 숨은 지도 자동 팬으로 타일 선적재, `clearTileCache`; 설정 카드는 app.js renderSettings) · `_esc`/`_escq` · `_showAdminDenied`
