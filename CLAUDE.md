...
- **app.js** (3.4k줄): 설정 `renderSettings` · **🧗암벽 이용관리**(`openClimb`/`climbUpload`→SheetJS 지연로드 `_loadXlsx`·헤더명기반 파서 `_climbParseWB`·이용일자별 저장 `_climbSave`→`climbUsage` 컬렉션+커버리지 `climbDates`, 홈카드 `_climbHomeCardHtml`/`_climbHomeNeed`, 통계 `_renderClimb`, 시즌 5.16~11.14 `_climbInSeason`, 열람권한 `_canClimb`=특수산악구조대·마스터·개발자, 코스→지구 `CLIMB_DISTRICTS`; 개인정보라 명단은 전체동기화 X) · 카카오 로그인/인증 · KMA 특보 수신(`_parseKmaWarnings` EUC-KR·코드포맷, `_SETAK_REGIONS` 지역필터, `kmaWarnDiag` 진단) · 24h 경과알림 `_checkStaleRescues`(작성자에게만) · 카톡 자기메시지 `_sendKakaoSelf` · 통계 · 응소 · 🆘SOS · **OTA(`OTA_VER` ~2787행 — deploy.sh가 자동 증가)** · `window.onload` 부팅
